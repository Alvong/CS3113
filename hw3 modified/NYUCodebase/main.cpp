...
public:
    ...
    void Init()
    {
        ...
        player player1(LoadTexture("/Users/Alvong/Documents/game programming/hw3/NYUCodebase/sheet.png"),pbullet,211.0f/1024.0f,941.0f/1024.0f,99.0f/1024.0f,75.0f/1024.0f,0.0,-0.8);
        firstplayer=player1;
...
//        }
    ...
    void ingame(float elapsed,SDL_Event event,const Uint8 *keys)
    {
        ...
        if (state==STATE_GAME) {
             ...
             if(keys[SDL_SCANCODE_LEFT])
            {
                ...
                {
                    firstplayer.x-=0.5*elapsed;
                    
                }
                else
                {
                    
                    firstplayer.x=-1+(firstplayer.width/2)*elapsed;
            ...
            if(keys[SDL_SCANCODE_UP] &&!firstplayer.fired)
                {
                firstplayer.fired=true;
            ...
            {
                one->y+=(0.7*one->direction)*elapsed;
            ...
            {
                if(limitx<0.2){one->x+=(0.08*one->direction)*elapsed;}
                if(limitx>=0.2&&limitx<0.6){one->x-=(0.1*one->direction)*elapsed;}
                if(limitx>=0.6&&limitx<1){one->x+=(0.1*one->direction)*elapsed;}
                if(limitx>1){one->x-=(0.1*one->direction)*elapsed;}
                one->y-=(0.04*one->direction)*elapsed;
